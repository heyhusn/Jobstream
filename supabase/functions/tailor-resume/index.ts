...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!jobId) {
      throw new UserFacing("That request didn't say which posting to tailor against.");
    }
    ...
    if (!resumeText) {
      throw new UserFacing("There's no resume text to tailor yet. Upload a resume first.");
    }

    const ctx: ResumeTailorContext = {
      resumeText,
      jobTitle: job.title,
    ...
    // ── persist ───────────────────────────────────────────────
    // No dedicated table — like resume_optimize/skill_gap, the
    // result lives on the task row, keyed by job_id in the input.
    const { error: doneErr } = await supabase
      ...
      .update({
        ...
        result: {
          ...
          not_addressed: result.notAddressed,
          model: MODEL,
    ...
    if (doneErr) {
      ...
      const { error: retryErr } = await supabase
        ...
        .update({
          status: "done",
          result: { job_id: jobId, rewrites: result.rewrites, not_addressed: result.notAddressed, model: MODEL },
        })
        .eq("id", taskId);
      if (retryErr) {
        console.error("second attempt to mark the task done failed:", retryErr);
        return json({ error: "The tailoring finished but the task could not be updated." }, 500);
      }
    }

    return json({ success: true, rewrites: result.rewrites.length, not_addressed: result.notAddressed.length });
  } catch (error) {
    let message =
...
async function callDeepSeek(
  ...
  try {
    ...
    // not — DeepSeek billed for this call either way.
    try {
