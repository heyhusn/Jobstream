...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── persist ───────────────────────────────────────────────
    // No dedicated table — the
    // result lives on the task row.
    ...
    if (doneErr) {
      ...
      const { error: retryErr } = await supabase
        ...
        .update({
          status: "done",
          result: { resume: result, model: MODEL },
        })
        .eq("id", taskId);
      if (retryErr) {
        console.error("second attempt to mark the task done failed:", retryErr);
        return json({ error: "The building finished but the task could not be updated." }, 500);
      }
    }
