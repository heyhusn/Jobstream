...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // for every caller, including the real admin — meaning every
    // action in this console (run ingestion, recompute ghost
    // signals, send the notification digest) has 401'd
    // unconditionally since this function was first deployed. Found
    // while testing an unrelated rate-limit feature (minor m37); see
