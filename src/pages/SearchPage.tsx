...
function SearchResultRow({ result }: { result: HybridSearchResult }) {
  return (
    <JobResultRow
      ...
      headerRight={<Badge tone="neutral">{matchedViaLabel[result.matched_via]}</Badge>}
      expandedTop={<p className="text-sm leading-relaxed text-ink-70">{result.job.description}</p>}
    />
  );
}
